...
class GitHubFetcher:
    ...
    async def fetch_repo_info(self, full_name: str) -> Optional[Dict]:
        """Fetch info for a specific repository by full name (owner/repo)"""
        try:
            ...
            logging.error(f"Failed to fetch repo {full_name}: {e}")
            return None
...
class ThemeScraperAgent:
    ...
    async def run(self, max_repos: int = None, resume: bool = True, custom_query: str = None,
        ...
        if not self.config.GITHUB_TOKEN:
            ...
            print("   Without this, you'll be limited to 60 requests/hour and may get rate limited immediately.")
            print("   Get a token at: https://github.com/settings/tokens")
            response = input("\nContinue anyway? (y/n): ")
            if response.lower() != 'y':
                print("Exiting. Please add GITHUB_TOKEN to your .env file.")
                return
        else:
            print("✅ GitHub token found")
        
        if self.analyzer.use_ai:
            print(f"✅ MiniMax AI enabled ({len([k for k in self.config.MINIMAX_API_KEYS if k])} keys)")
        else:
            print("ℹ️  AI disabled - using heuristic categorization")
        
        # Check if using input file
        if input_file:
            print(f"📄 Reading repos from: {input_file}")
            repo_list = self._read_repo_file(input_file)
            if not repo_list:
                print(f"❌ ERROR: Could not read repos from {input_file}")
                return
            ...
            print(f"📊 Queries: {len(self.config.DEFAULT_QUERIES if not custom_query else [custom_query])}")
        ...
        try:
            ...
            if input_file and repo_list:
                # Process from file
                await self._process_from_file(repo_list, processed, pbar)
            else:
                # Process from search queries
    ...
    def _setup_signal_handlers(self):
        ...
        def signal_handler(sig, frame):
            ...
            logging.warning("Shutdown signal received - forcing exit")
    ...
    def _read_repo_file(self, filepath: str) -> List[str]:
        """Read repository names/URLs from a text file"""
        try:
    ...
    async def _process_from_file(self, repo_list: List[str], processed: Set[str], pbar):
        """Process repositories from a pre-defined list"""
        batch = []
        total_new = 0
        
        for repo_full_name in repo_list:
            if repo_full_name in processed:
                pbar.update(1)
                continue
            
            # Fetch repo info directly
            repo_info = await self.fetcher.fetch_repo_info(repo_full_name)
            if not repo_info:
                logging.warning(f"Could not fetch info for {repo_full_name}")
                pbar.update(1)
                continue
...
def main():
    ...
    parser.add_argument('--input-file', type=str, help="Read repos from text file (one repo per line)")
